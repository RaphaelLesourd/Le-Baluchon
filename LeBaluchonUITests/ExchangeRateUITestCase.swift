...
class ExchangeRateUITestCase: XCTestCase {
    ...
    func testExchangeTabViews() {
        ...
        let orignCurrencyView = element.children(matching: .other).element(boundBy: 1).children(matching: .other).element(boundBy: 0)
        let destinationCurrencyView = element.children(matching: .other).element(boundBy: 2).children(matching: .other).element(boundBy: 0)
        let rateView = element.children(matching: .other).element(boundBy: 3).children(matching: .other).element(boundBy: 0)
