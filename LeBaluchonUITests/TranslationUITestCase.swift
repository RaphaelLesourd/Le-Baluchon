...
class TranslationUITestCase: XCTestCase {
    ...
    func testTranslationTabViews() {
        ...
        let originLanguague = elementsQuery.buttons["Français"].staticTexts["Français"]

        let destinationLanguage = elementsQuery.staticTexts["Anglais"]

        let languageSwapButton = elementsQuery.buttons["Arrow Right Circle"]

        let originTextView = elementsQuery.textViews
            .containing(.staticText, identifier: "Saisissez votre texte")
            .element
        let translatedTextView =  app.scrollViews
            .containing(.other, identifier: "Horizontal scroll bar, 1 page")
